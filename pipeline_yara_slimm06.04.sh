...
RUNDIR=krawczyk/workdir/Virusseq/fastq
OUTDIR=yara_slimm_fastq_results
SLIMM_DB=krawczyk/workdir/databases/slimm_db_C-RVDB16.0_HUMAN_GENOME.sldb
YARA_DB=krawczyk/workdir/databases/C-RVDBv16.yara.index
BASEN=${RUNDIR##*/}
...
for FR1 in ${FQ}; do
   ...
   > ${SAMPLEN}.total.time.log 
   done

###6. Running multiqc
echo "[$(date)]: Starting multiqc" >&2
mkdir $OUTDIR/multiqc_output
